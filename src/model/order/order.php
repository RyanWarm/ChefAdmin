...
class ModelOrderOrder extends Model {
	...
	public function edit($id, $data) {
		...
		if( !empty($data['image']) && strpos($data['image'], 'recruit/') === false ) { 
			$link = $this->uploadImage($data['image']);	
			if( !empty($link) && $link != '' ) {
				$updateStr = "UPDATE orders SET image = '" . $link . "' WHERE id = " . (int)$id;
				$this->db->query($updateStr);
			}
		} else if( empty($data['image']) ) {
			//reset image
			$updateStr = "UPDATE orders SET image = '' WHERE id = " . (int)$id;
			$this->db->query($updateStr);
		}
		*/
		$updateStr = "UPDATE orders SET alias = '" . $this->db->escape($data['alias']) . "'";
